...
// 上传队列中所有文件项对应上述代码中的 uploadDataList: UploaderData[] 数组, 里面包含每个上传项各自的暂停、恢复、移除、更新操作，分别是 uploadDataItem: UploaderData 里的 pause、resume、remove、update方法。
// 其中 update 方法，用来更新该项的各种状态属性（即 UploaderData 类型），正常情况下不需要使用，因为已经由库帮你管理了。只有两种情况需要：

// 你关闭了切片功能，因此 chunks 数组里只有一个完整的文件: file，相当于只有 1 个切片，由于 upload 请求完全交给了开发者做，库本身是不知道单个切片内部的上传进度的，这时开发者就可以使用 update({ progress: number }) 手动更新进度。
